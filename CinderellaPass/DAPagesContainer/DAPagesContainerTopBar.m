...
- (void)setItemTitles:(NSArray *)itemTitles
{
    if (_itemTitles != itemTitles) {
        ...
        for (NSUInteger i = 0; i < itemTitles.count; i++) {
            ...
            NSData *data = [NSData dataWithContentsOfURL:[NSURL URLWithString: [NSString stringWithFormat:@"%@%@category%d.png",DOMAIN_NAME,WEB_LINK_TOP_CATEGORY,i]]];
            NSData *data_selected = [NSData dataWithContentsOfURL:[NSURL URLWithString: [NSString stringWithFormat:@"%@%@category%d-selected.png",DOMAIN_NAME,WEB_LINK_TOP_CATEGORY,i]]];
            UIImage *image;
            UIImage *image_selected;
            if([data length] && [data_selected length]){
                image = [UIImage imageWithData: data];
                image_selected = [UIImage imageWithData: data_selected];
                [itemView setImage:image forState:UIControlStateNormal];
                [itemView setImage:image_selected forState:UIControlStateHighlighted];
...
{
//    CGRect frame = CGRectMake(0., 0., 108.0f, 50.0f);
...
//        CGFloat width = [self.itemTitles[i] sizeWithFont:self.font].width;
        CGFloat width = DAPagesContainerTopBarItemViewWidth;
